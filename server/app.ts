...
import { sql, testConnection, isDbConfigured } from './db';
...
app.get(['/health', '/api/health'], async (_req, res) => {
  ...
  if (conn.ok) {
    ...
    res.status(conn.error?.includes('não configurada') ? 200 : 500).json({ 
      status: 'warning', 
      neon: 'disconnected', 
      message: conn.error 
    });
...
app.get(['/bootstrap', '/api/bootstrap'], async (_req, res) => {
  ...
  if (!isDbConfigured) {
    return res.json({
      ok: false,
      neon: 'disconnected',
      message: 'DATABASE_URL não configurada no ambiente. Usando dados locais.'
    });
  }
  ...
  try {
    ...
    const companies = rawCompanies.map((comp: any) => {
